...
## 1. Modifying bits.c and Checking for Compliance
...
Use the `dlc` compiler to automatically check your version of `bits.c` for compliance with the coding guidelines:
```bash
./dlc bits.c
```

`dlc` returns silently if there are no problems with your code. Otherwise it prints messages that flag any problems.
...
To compile and run the btest program:
```bash
make btest
./btest [optional cmd line args]
```

You will need to recompile `btest` each time you change your `bits.c` program. When moving from one platform to another, you will want to get rid of the old version of `btest` and generate a new one:
```bash
make clean
make btest
```
...
**Note:** `btest` does not check your code for compliance with the coding guidelines. Use `dlc` to do that.
